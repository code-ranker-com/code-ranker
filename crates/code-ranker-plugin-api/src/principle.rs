//! The Prompt-Generator [`Principle`] DTO.
//!
//! A `Principle` is **prompt-generator domain data**, not part of the parser
//! contract: a plugin *produces* its set (via
//! [`LanguagePlugin::principles`](crate::plugin::LanguagePlugin::principles)), but every
//! other consumer — the report snapshot, the `recommend` console/prompt views —
//! only *reads* principles and never parses anything. The type therefore lives here,
...
/// [`Principle`] in — the framing prose around a principle (intro, the doc-read
...
/// `task` line is substituted with the active principle id at render time.
...
/// A Prompt-Generator principle (a refactoring principle): a ready-to-paste AI
/// instruction plus how the UI seeds the node selection for it. Each plugin
/// builds its own set from config via [`LanguagePlugin::principles`](crate::plugin::LanguagePlugin::principles)
/// (the common catalog plus any language-specific principles).
...
pub struct Principle {
    ...
    /// Which connection sets the principle pre-selects: any of `"in"`/`"out"`/`"common"`.
